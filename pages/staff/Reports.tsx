...
const StaffReports: React.FC = () => {
    ...
    // Define fetch logic out of effects for useSyncData
    const fetchReport = async () => {
        ...
        try {
            ...
            const displayDetails = allMyRecords.filter(d => d.status === AttendanceStatus.ABSENT || d.status === AttendanceStatus.LATE);
    ...
    useSyncData('attendance', () => {
        if (activeTab === 'daily') fetchReport();
        else fetchStats();
    });
                                                ...
                                                {reportData.details.map((d, idx) => {
                                                    ...
                                                    return (
                                                        <tr key={idx} className="hover:bg-slate-50 transition-colors">
                                                            ...
                                                            <td className="p-4">
                                                                <span className={`px-3 py-1 rounded-full text-xs font-bold ${d.status === AttendanceStatus.ABSENT ? 'bg-red-100 text-red-700' : 'bg-amber-100 text-amber-700'
                                                                    }`}>
                                                                    {d.status === AttendanceStatus.ABSENT ? 'غائب' : 'متأخر'}
                                                                </span>
                                                            </td>
                                                            <td className="p-4">
                                                                {status ? (
                                                                    <span className={`flex items-center gap-1.5 w-fit px-3 py-1.5 rounded-lg text-xs font-bold border ${status === RequestStatus.APPROVED ? 'bg-emerald-50 text-emerald-700 border-emerald-200' :
                                                                        status === RequestStatus.REJECTED ? 'bg-red-50 text-red-700 border-red-200' :
                                                                            'bg-amber-50 text-amber-700 border-amber-200'
                                                                        }`}>
                                                                        <div className={`w-1.5 h-1.5 rounded-full ${status === RequestStatus.APPROVED ? 'bg-emerald-500' :
                                        ...
                                        {dailyClassSummary.map((item, idx) => (
                                            <div key={idx} className="bg-white border border-slate-200 rounded-xl p-4 hover:border-blue-300 transition-colors">
                                                <div className="flex justify-between items-start mb-3">
                                                    ...
                                                    <span className={`text-xs font-bold px-2 py-1 rounded-lg ${item.rate > 90 ? 'bg-emerald-100 text-emerald-700' :
                                                ...
                                                <div className="w-full bg-slate-100 rounded-full h-2 mb-3">
                                                    <div className={`h-2 rounded-full ${item.rate > 90 ? 'bg-emerald-500' :
                                                ...
                                                <div className="relative z-10">
                                                    ...
                                                    <div className="mb-4">
                                                        ...
                                                        <div className="w-full bg-slate-100 rounded-full h-2.5">
                                                            <div
                                                                className={`h-2.5 rounded-full transition-all duration-1000 ${cls.rate >= 90 ? 'bg-gradient-to-r from-emerald-400 to-emerald-600' :
