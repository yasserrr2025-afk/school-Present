                ...
                {!isEditing && (
                  <div className="bg-slate-50 p-4 rounded-2xl border border-slate-100 space-y-4">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      <select value={formGrade} onChange={e => { setFormGrade(e.target.value); setFormClass(''); }} className="w-full p-3 border-none rounded-xl bg-white text-sm font-bold shadow-sm focus:ring-2 focus:ring-pink-100 outline-none"><option value="">الصف</option>{uniqueGrades.map(g => <option key={g} value={g}>{g}</option>)}</select>
